...
## Introduction
This is an RGB sandbox and demo based on rgb-node version 0.4.2.
It is based on the original rgb-node demo by
[St333p](https://github.com/St333p) (version 0.1) and
[grunch](https://github.com/grunch)'s
[guide](https://grunch.dev/blog/rgbnode-tutorial/).

Please note that future rgb-node versions will be incompatible with this one.

It runs in Docker using Rust 1.60.0 and Debian bullseye. The underlying Bitcoin
network is `regtest`.

This sandbox can help explore rgb-node features in a self-contained environment
...
## Setup
Some initial steps are required in order to make the sandbox operational.
...
### Service management
Clone the repository and start the required services in Docker containers:
```bash=
...
Check out the manual demo below to get started with examples commands. Refer to
